...
void args_to_options(options_t* options) {
  ...
  options->twitter = args.twitter_given;

  if (args.twitter_given && args.twemcache_path_given)
    options->twemcache_path = args.twemcache_path_arg;
  else {
    fprintf(stderr, "twemcache_path must be set when using --twitter option");
    exit(1);
  }
