...
int main(int argc, char **argv) {
  // hrlee: Save the pid of mutilate to a file so that it can be used to terminate
  // mutilate by a remote server that is running Datamime.
  const char *tid_filename = "/scratch/hrlee/mutilate.tid";
  FILE *tidfile =  fopen(tid_filename, "w");
  fprintf(tidfile, "%d", gettid());
  fclose(tidfile);
...
void args_to_options(options_t* options) {
  ...
  // hrlee : If QPS is not given, lambda becomes zero which means that the master
  // and all agents are set to max QPS. Since for our use case we do _not_ use
  // agent mode (single-machine setup, where network latency is minimal),
  // We set lambda = 1 for the master and don't care about the agent lambda setup.
  // TODO: If we actually want to do the _correct_ thing here, one approach is to
  // Calculate the expected average qps from args->ia.
  if (args.qps_given)
    options->lambda = (double) options->qps / (double) options->lambda_denom * args.lambda_mul_arg;
  else
    options->lambda = 1; // any positive value

  V("qps_given:%d, connections_given:%d, connections_made:%d, lambda:%f",
  options->qps, options->connections, connections, options->lambda);
  ...
  D("options->ia = %s", args.iadist_arg);
