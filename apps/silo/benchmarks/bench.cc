...
bench_runner::run()
{
  // hrlee: Why are we initializing here? the whole server basically waits here
  // until it establishes connection(s) with the client(s), which means that
  // the server will be busy initially while the client(s) send all the requests,
  // messing up queueing time...
  ...
  // Initialize server as close to when we can accept requests as possible.
  tBenchServerInit(nthreads);
